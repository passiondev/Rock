...
{
    [DisplayName( "Dynamic Group Registration" )]
    [Category( "CRM" )]
...
", CodeEditorMode.Lava, CodeEditorTheme.Rock, 200, true, "", "", 9 )]
    ...
    [GroupTypeField("Group Type", "Group Type of group that people will be added to.", true, null, null, 11, GROUP_TYPE_KEY, null)]
    [GroupField("Parent Group", "The parent group of the group that will be created.", true, null, null, 12, GROUP_KEY)]
    [GroupRoleField(null, "Primary Group Role", "Group Role of the Primary Person.", true, null, null, 13, PRIMARY_ROLE_KEY)]
    [GroupRoleField(null, "Secondary Group Role", "Group Role of the secondary people.", true, null, null, 14, SECONDARY_ROLE_KEY)]
    [TextField("Main Header", "The main header for the page.", false, "", null, 15, MAIN_HEADER)]
    [TextField("Primary Person Header", "The header for the primary person section of the page.", false, "", null, 16, PRIMARY_PERSON_HEADER)]
    [TextField("Secondary Person Singular", "The singular term for the secondary person.", false, "", null, 17, SECONDARY_PERSON_SINGULAR)]
    [TextField("Secondary Person Plural", "The plural term for the secondary person.", false, "", null, 18, SECONDARY_PERSON_PLURAL)]
...
ORDER BY [Text]", false, "", "Child Relationship", 2, "CanCheckinRelationships" )]
    ...
    public partial class DynamicGroupRegistration : RockBlock
    {
        ...
        private const string GROUP_TYPE_KEY = "GroupTypeKey";
        private const string GROUP_KEY = "GroupKey";
        private const string PRIMARY_ROLE_KEY = "PrimaryRole";
        private const string SECONDARY_ROLE_KEY = "SecondaryRole";

        private const string MAIN_HEADER = "MainHeader";
        private const string PRIMARY_PERSON_HEADER = "PrimaryHeader";
        private const string SECONDARY_PERSON_SINGULAR = "SecondaryPersonSingular";
        private const string SECONDARY_PERSON_PLURAL = "SecondaryPersonPlural";
        ...
        /// The group members.
        /// </value>
        protected List<PreRegistrationPerson> Children { get; set; }
        ...
        protected override void LoadViewState( object savedState )
        {
            ...
            Children = ViewState["Children"] as List<PreRegistrationPerson> ?? new List<PreRegistrationPerson>();
...
",
        ...
        {
            var row = sender as PreRegistrationPersonRow;
        ...
        protected void btnSave_Click( object sender, EventArgs e )
        {
            if ( ValidateInfo() )
            {
                ...
                var familyGroupType = GroupTypeCache.Get(GetAttributeValue(GROUP_TYPE_KEY));
                var primaryRoleId = familyGroupType.Roles.FirstOrDefault( r => r.Guid == GetAttributeValue(PRIMARY_ROLE_KEY).AsGuidOrNull() ).Id;
                var childRoleId = familyGroupType.Roles.FirstOrDefault(r => r.Guid == GetAttributeValue(SECONDARY_ROLE_KEY).AsGuidOrNull()).Id;
                var parentGroupId = new GroupService(new RockContext()).GetByGuid(GetAttributeValue(GROUP_KEY).AsGuid()).Id;
                ...
                Guid? familyGuid = hfFamilyGuid.Value.AsGuidOrNull(); //hfFamilyGuid is hidden field in ascx file. 
                //if ( familyGuid.HasValue )
                //{
                //    primaryFamily = groupService.Get( familyGuid.Value );     // May use later if we check for existing groups
                //}
                ...
                foreach ( var child in Children )
                {
                    ...
                    //     EnsurePersonInOtherFamily( familyGroupType.Id, primaryFamily.Id, parentGroupId, person.Id, person.LastName, childRoleId, newFamilyIds );
                    //
                    ...
                    groupMember.GroupId = primaryFamily.Id;
                ...
                //     {
                //         EnsurePersonInOtherFamily( familyGroupType.Id, primaryFamily.Id, parentGroupId, removedChild.Id, removedChild.Person.LastName, childRoleId, na );
        ...
        private void SetCurrentFamilyValues()
        {
            ...
            Children = new List<PreRegistrationPerson>();
            ...
            //    {
            //        var child = new PreRegistrationPerson( groupMember.Person );
        ...
        private void AddChild()
        {
            ...
            var child = new PreRegistrationPerson( person );
        ...
        {
            prChildren.ChildSingular = GetAttributeValue(SECONDARY_PERSON_SINGULAR);
            prChildren.ChildPlural = GetAttributeValue(SECONDARY_PERSON_PLURAL);
            mainHeader.InnerText = GetAttributeValue(MAIN_HEADER);
            primaryPersonHeader.InnerText = GetAttributeValue(PRIMARY_PERSON_HEADER);
            secondaryPersonHeader.InnerText = GetAttributeValue(SECONDARY_PERSON_PLURAL);
        }
        ...
        private void CreateChildrenControls( bool setSelection )
        {
            ...
            foreach ( var child in Children )
            {
                if ( child != null )
                {
                    var childRow = new PreRegistrationPersonRow();
            ...
            DynamicPlaceholder phAttributes )
        {
            ...
            if ( adult != null || ( tbFirstName.Text.IsNotNullOrWhiteSpace() && tbLastName.Text.IsNotNullOrWhiteSpace() ) )
            {
                ...
                //var familyGroupTypeGuid = GroupTypeCache.Get(Rock.SystemGuid.GroupType.GROUPTYPE_FAMILY.AsGuid());
                //var family = new Group();
                //family.Name = adult.LastName.FixCase() + " Family";
                //family.GroupTypeId = familyGroupTypeGuid.Id;
                ...
                //// Make sure they're added to a family
                //var groupMemberService = new GroupMemberService(_rockContext);
                //var familyMember = new GroupMember();
                //familyMember.GroupId = family.Id;
                //familyMember.GroupRoleId = adultRoleId;
                //familyMember.PersonId = adult.Id;
                //familyMember.GroupMemberStatus = GroupMemberStatus.Active;
        ...
        private bool ValidateInfo()
        {
            ...
            {
                errorMessages.Add( "The name of a primary contact needs to be entered." );
            }
        ...
        /// </summary>
        /// <param name="groupTypeId">The family group type identifier.</param>
        /// <returns></returns>
        private Group CreateNewFamily( int groupTypeId, string lastName, int parentGroupId )
        {
            // If we don't have an existing family, create a new family
            var group = new Group();
            group.Name = lastName.FixCase() + " Group";
            ...
            else
            {
                ...
                if ( campusGuid.HasValue )
                {
                    ...
                    if ( defaultCampus != null )
                    {
                        group.CampusId = defaultCampus.Id;
                    }
                }
            }
        ...
        private void EnsurePersonInOtherFamily( int familyGroupTypeId, int primaryfamilyId, int parentGroupId, int personId, string lastName, int childRoleId, Dictionary<string, int> newFamilyIds )
        {
            var groupMemberService = new GroupMemberService( _rockContext );
            ...
            {
                var family = CreateNewFamily( familyGroupTypeId, lastName, parentGroupId );
