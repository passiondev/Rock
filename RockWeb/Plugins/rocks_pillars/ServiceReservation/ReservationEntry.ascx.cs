...
namespace RockWeb.Plugins.rocks_pillars.ServiceReservation
{
    ...
    [IntegerField("Minutes After Start", "The number of minutes after a service starts that people can still make a reservation.", false, 0, "", 1 )]
    [LinkedPage( "Workflow Entry Page", "The page to redirect user to once they've selected a service.", false, "", "", 2 )]
    [WorkflowTypeField( "Reservation Workflow", "The workflow type that is used to track registrations. This workflow must have the following attribute keys: Campus, Location, Schedule, Date, HowMany", false, true, "", "", 3 )]
    [LocationField( "Location Filter", "Optionally limit any reservations to only this location.", false, "", "", 4, "Location" )]
    [SchedulesField( "Schedules Filter", "Optionally limit any reservations to only these schedules.", false, "", "", 5, "Schedules" )]
    [TextField( "Location Order Attribute Key", "By default, locations will display in order by name. If you have a location attribute that should control order, enter that attribute key here.", false, "", "", 6 )]
    [BooleanField( "Single Schedule Skip", "True", "False", "If there is only one schedule available, the schedule selection screen will be skipped.", false, "", 7 )]
    [BooleanField( "Enable Campus Context", "True", "False", "If this is enabled it will get the Campus from the Context of the users browser", false, "", 8 )]
    [LavaCommandsField( "Enabled Lava Commands", "The Lava commands that should be enabled for the card formats.", false, "", "", 9 )]
...
", "Formatting", 12 )]
    ...
    public partial class ReservationEntry : Rock.Web.UI.RockBlock
    {
        ...
        private string _capacitiesAttrKey = string.Empty;
        private string _attMatrixSchCapKey = string.Empty;
        ...
        private void RptrCards_ItemCommand( object source, RepeaterCommandEventArgs e )
        {
            ...
            if ( guid.HasValue )
            {
                ...
                if ( campus != null && campus.Guid == guid.Value ) // Adding check for guid due to SV issue where campus was cached with a Schedule's guid.
                {
                    CampusSelected = true;
        ...
        {
            _commonMergeFields = Rock.Lava.LavaHelper.GetCommonMergeFields( this.RockPage, null );

            var schedMatrixItemAtt = AttributeCache.Get( rocks.pillars.ServiceReservation.SystemGuid.Attribute.SCHEDULE_ATTRIBUTE_MATRIX_ITEM );
            var capMatrixItemAtt = AttributeCache.Get( rocks.pillars.ServiceReservation.SystemGuid.Attribute.CAPACITY_ATTRIBUTE_MATRIX_ITEM );
            var locSchCapAttMtx = AttributeCache.Get( rocks.pillars.ServiceReservation.SystemGuid.Attribute.LOCATION_SCHEDULE_CAPACITIES_ATTRIBUTE );

            if (  schedMatrixItemAtt == null || locSchCapAttMtx == null)
            {
                throw new Exception( "The Location Schedules attribute was not defined." );
            }
            _schedulesAttrKey = schedMatrixItemAtt.Key;
            _capacitiesAttrKey = capMatrixItemAtt.Key;
            _attMatrixSchCapKey = locSchCapAttMtx.Key;
            ...
            var minsAfterStart = GetAttributeValue( "MinutesAfterStart" ).AsInteger();
            ...
            using ( var rockContext = new RockContext() )
            {
                ...
                foreach (var attributeValue in new AttributeValueService(rockContext)
                    ...
                    .Where(v => v.AttributeId == schedMatrixItemAtt.Id)
                    .Select(v => v.Value))
                {
                    var scheduleGuid = attributeValue.AsGuid();

                    if (!validSchedules.Any() || validSchedules.Contains(scheduleGuid))
                    {
                        scheduleDates.TryAdd(scheduleGuid, new List<DateTime>());
                    }
                }
                ...
                // Adjust start/end to be valid
                var minStartDate = RockDateTime.Now.AddMinutes( 0 - minsAfterStart );
                    ...
                    .Where( s => scheduleGuids.Contains( s.Guid ) ) )
                {
                    ...
                    {
                        if ( occurrence.Period.StartTime.Value >= minStartDate )
                        {
                ...
                var attributeMatrixQry = new AttributeMatrixService( rockContext )
                    .Queryable().AsNoTracking();
                ...
                foreach ( var campus in campuses )
                {
                    if ( ( campus.IsActive ?? true ) && campus.LocationId.HasValue )
                    {
                        ...
                        if ( campusLocation != null )
                        {
                            ...
                            locations.LoadAttributes(rockContext);

                            foreach ( var location in locations )
                            {
                                if ( !validLocation.HasValue || location.Guid == validLocation.Value )
                                {
                                    var attMatrixGuid = location.GetAttributeValue(_attMatrixSchCapKey).AsGuid();
                                    var attMatrix = attributeMatrixQry.Where(m => m.Guid == attMatrixGuid).FirstOrDefault();

                                    if(attMatrix != null)
                                    {
                                        foreach(var matrixItem in attMatrix.AttributeMatrixItems)
                                        {
                                            matrixItem.LoadAttributes( rockContext );
                                            var scheduleGuid = matrixItem.GetAttributeValue(_schedulesAttrKey).AsGuid();
                                            var capacity = matrixItem.GetAttributeValue(_capacitiesAttrKey).AsIntegerOrNull();
