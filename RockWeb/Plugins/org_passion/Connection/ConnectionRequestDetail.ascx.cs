        ...
        public void RebindGroupsAndConnectors(ConnectionRequest connectionRequest, RockContext rockContext)
        {
            ...
            if (connectionRequest != null)
            {
                ...
                foreach (var g in groups.OrderBy(g => g.Name))
                {
                    ...
                    ddlPlacementGroup.Items.Add(new ListItem(String.Format("{0} - {1} ({2})", g.ParentGroup != null ? g.ParentGroup.Name : "", g.Name, g.Campus != null ? g.Campus.Name : "No Campus"), g.Id.ToString().ToUpper()));
