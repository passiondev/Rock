...
{
    [DisplayName("My Well Gateway Apple Pay List")]
    [Category("My Well > Gateway")]
    [Description("My Well Gateway Apple Pay List.")]
    [LinkedPage("Detail Page", order: 0)]

    public partial class ApplePayList : RockBlock, ICustomGridColumns
    {
        ...
        protected void gApplePayList_Delete(object sender, RowEventArgs e)
        {
            ...
            var applePayDomain = applePayDomainService.Get(e.RowKeyId);
            ...
            if (applePayDomain != null)
            {
                rockContext.WrapTransaction(() =>
               {
                   ...
                   String[] domains = { applePayDomain.DomainName };

                   var applePayData = new CreateApplePayDomainData
                   {
                       DomainNames = domains,
                       BenevityId = applePayDomain.BenevityId
                   };

                   // When deleting a domain is requested we will unregister the domain with apple
                   var response = myWellGateway.UnregisterApplePayDomains(applePayDomain.FinancialGateway, applePayData, out errorMessage);

                   if (response == false || errorMessage.IsNotNullOrWhiteSpace())
                   {
                       nbWarningMessage.Text = "Failed to delete the apple pay domain configuration.";
                       return;
                   }

                   applePayDomainService.Delete(applePayDomain);
        ...
        /// <summary>
        /// Handles the RowSelected event of the gApplePayList control.
        /// </summary>
        /// <param name="sender">The source of the event.</param>
        /// <param name="e">The <see cref="RowEventArgs"/> instance containing the event data.</param>
        protected void gApplePayList_Edit(object sender, RowEventArgs e)
        {
            NavigateToLinkedPage("DetailPage", "ApplePayDomainId", e.RowKeyId);
        }
        ...
        {
            NavigateToLinkedPage("DetailPage", "ApplePayDomainId", 0);
        }
        ...
        private void gApplePayList_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            ...
            {
                var applePayDomain = e.Row.DataItem as MyWellGatewayApplePayDomain;
        ...
        protected void lVerifiedStatus_DataBound(object sender, RowEventArgs e)
        {
            ...
            lVerificationtStatus.Text = string.Format("<span class='{0}'>{1}</span>", getStatusLabelClass(importRow.IsVerified), getStatusText(importRow.IsVerified));
        }

        /// <summary>
        /// Binds the grid.
        /// </summary>
        private string getStatusLabelClass(bool isVerified)
        {
            if (isVerified)
            {
        ...
        /// Binds the grid.
        /// </summary>
        private string getStatusText(bool isVerified)
        {
            if (isVerified)
            {
                return "Verified";
        ...
        {
            var queryable = new MyWellGatewayApplePayDomainService(new RockContext()).Queryable().OrderBy(a => a.Id);
