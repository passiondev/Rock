...
How a header image gets onto a user-facing Form Builder form, what already
exists in production, and what changes to make.
...
## Change 2 — Setting a header image today, before any deploy

The Form Builder Settings tab does not have the upload field yet (that needs
Change 3, which needs a build). Until then, staff can set the image through the
generic workflow-type editor:
...
## Change 3 — The Settings tab field (needs a build)
...
Implemented in the repo on branch `develop-18.3`, four files:
...
Obsidian block, so it ships with the next full build and deploy.

No EF migration was added, deliberately — production already has the attribute,
and a migration in core `Rock.Migrations` would collide on the eventual 18.3 →
18.4 upstream merge. If this ever needs to run on an installation without the
attribute, provision it there rather than in a core migration.
...
| Change 3 — Settings tab | **Yes** | Upload from Form Builder instead of the workflow editor |
