...
Add server scripts and GitHub workflow handling for `rock:stop` and `rock:destroy`, plus PR close/merge behavior.

## Acceptance criteria

- [x] Adding `rock:stop` stops the PR app pool/site but preserves files, IIS config, and environment manifest.
- [x] Adding `rock:destroy` removes the PR IIS site, app pool, deployed files, and manifest.
...
- [x] Reopened PRs can be started again with `rock:start`.
...
- `.github/workflows/pr-test-lifecycle.yml` handles `rock:stop`, `rock:destroy`, merged PR destroy, and closed-unmerged PR stop, then updates the sticky PR status comment.
