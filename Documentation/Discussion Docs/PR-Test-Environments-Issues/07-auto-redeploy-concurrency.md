...
Add optional auto-redeploy behavior for PRs labeled `rock:auto`, and ensure concurrent deploys for the same PR use latest-commit-wins semantics.

## Acceptance criteria

- [x] PRs with `rock:auto` rebuild/redeploy when new commits are pushed.
- [x] PRs without `rock:auto` do not auto-redeploy on push.
- [x] Manual `rock:start` still redeploys latest PR head regardless of `rock:auto`.
...
- `.github/workflows/pr-test-deploy.yml` now also handles PR `synchronize` events, deploys only when `rock:auto` is present, and keeps PR-scoped `cancel-in-progress` concurrency.
