...
2. Scan the new catalog for legacy `text`/`ntext`/`image` columns: dispatch **DB - Find legacy text columns**, setting `db_name` to the catalog you are about to move (leave it empty and it scans the shared sandbox one instead). It is read-only. Do not reach for `Deployment/Database/Find-LegacyTextColumns.ps1` directly -- Cloud SQL here is Private Service Connect only, so the deploy VM is the only host that can reach the catalog, and that workflow is how you get the script onto it. `workflow_dispatch` runs the workflow file from the **default branch**, so do this before the cutover, not after: once the default moves, a workflow that was not carried across is unreachable (item 26).
