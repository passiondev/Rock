...
1. ~~**Decide where the catalog lives, and check the refresh mechanism first.**~~ **Resolved 2026-08-17 — `RockStaging` on `connect-restore-test` is fine.** The refresh is a per-database `.bak` import (`IMPORT` with `database: RockConnectProd`, preceded by a `DELETE_DATABASE`), not an instance-level backup restore, so a second catalog beside it survives. No `RESTORE_VOLUME` has ever run on this instance. **Disk checked 2026-08-18 — it fits, but turn autoresize on first.** Measured on the instance: 410.4 GiB of quota, 120.0 GiB used, so 290.4 GiB free. Seeding `RockStaging` from a current prod backup costs about 160 GiB — prod's own consumption today, not the ~127 GB the striped `.bak` suggests — which lands the instance near 280 GiB and leaves ~130 GiB. That is enough disk. What is not safe is doing it with `storageAutoResize` still **disabled**: the 18.4 → 19.3.4 migration runs at `Application_Start` and generates transaction log against a ~160 GiB catalog, and filling the disk mid-migration takes the instance read-only with the schema half-rewritten and no way back short of a restore. Enable autoresize with a limit, or bump the disk, before step 2. The tier is the tighter constraint than the disk in any case — `db-custom-2-8192` is 2 vCPU / 8 GB against prod's 8 vCPU / 32 GB, so expect the migration to be slow and size the deploy health-check timeout for it rather than the other way round.
